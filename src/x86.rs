            ...
            loop {
                ...
                loop {
                    ...
                    loop {
                        ...
                        let pyhs_addr = phys + addr - virt_start;
                        pte.set_page(pyhs_addr, attr)?;
...
const _: () = assert!(size_of::<GereralRegisterContext>() == (16 - 1) * 8);
#[allow(dead_code)]
#[repr(C)]
    ...
    unsafe {
        with_current_page_table(|pt| {
            pt.create_mapping(vstart, vend, vstart, PageAttr::ReadWriteIo)
                .expect("Failed to create mapping for MMIO region");
        });
    }
}
