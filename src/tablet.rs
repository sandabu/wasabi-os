...
use crate::usb::*;
use crate::xhci::{CommandRing, Controller};
use crate::{info, warn};
...
fn parse_hid_report_descriptor(report: &[u8]) -> Result<Vec<UsbHidReportInputItem>> {
    ...
    while let Some(prefix) = it.next() {
        ...
        let b_type = match (prefix >> 2) & 0b11 {
            ...
            _ => {
                warn!("b_type == Reserved is not impletmented yet!");
                break;
            }
        };
        let b_tag = prefix >> 4;
        ...
        match (&b_type, &b_tag) {
            ...
            (UsbHidReportItemType::Main, 0b1110) => {
                info!("M: }} Collection",)
            }
...
pub async fn start_usb_tablet(
    ...
    if device_descriptor.device_class != 0
        ...
        || device_descriptor.vendor_id != 0x627
        || device_descriptor.product_id != 0x0001
    {
        return Err("Not a USB Tablet");
    }
    let (_, interface_desc, other_desc_list) = pick_interface_with_triple(descriptors, (3, 0, 0))
