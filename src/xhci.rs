...
impl PciXhciDriver {
                ...
                let vendor = if device_descriptor.manufacture_idx != 0 {
                    Some(
                        Self::request_string_descriptor(
                            ...
                            &mut ctrl_ep_ring,
                            lang_id,
                            device_descriptor.manufacture_idx,
                        )
                        .await?,
                    )
                } else {
    ...
    async fn request_config_descriptor_and_rest(
        ...
        .await?;
        let itr = DescriptorIterator::new(&buf);
        let descriptors: Vec<UsbDescriptor> = itr.collect();
        Ok(descriptors)
    }
}
...
impl SetupStageTrb {
    ...
    pub const REQ_GET_CONFIGURATION: u8 = 9;
    pub const REQ_GET_INTERFACE: u8 = 11;
    pub const REQ_GET_PROTOCOL: u8 = 0x0b;
