...
pub struct TimerRegister {
    configuration_and_capacity: u64,
    _reserved: [u64; 3],
}
const _: () = assert!(size_of::<TimerRegister>() == 0x20);
impl TimerRegister {
    unsafe fn write_config(&mut self, config: u64) {
        write_volatile(&mut self.configuration_and_capacity, config);
    }
}
    ...
    pub fn new(registers: &'static mut HpetRegisters) -> Self {
        ...
        unsafe {
            ...
            for i in 0..hpet.num_of_timers {
                let timet = &mut hpet.registers.timers[i];
                let mut config = read_volatile(&timet.configuration_and_capacity);
                config &= !(TIMER_CONFIG_INT_ENABLE
                    | TIMER_CONFIG_USE_PERIODIC_MODE
                    | TIMER_CONFIG_LEVEL_TRIGGER
                    | (0b1111 << 9));
                timet.write_config(config);
            }
